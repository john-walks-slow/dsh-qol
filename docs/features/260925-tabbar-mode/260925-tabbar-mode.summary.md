...
用户确认的两个设计决策（ask_user_question）：

- **排序语义 = 混排**：黄（待处理）/绿（完成未读）/蓝（运行中）三种彩色状态**不分色**、按 updatedAt 混排在前；灰色空闲会话排后。非"黄>绿>蓝"严格分级。
...
3. **最近活跃模式**：
   ...
   - 排序 = rankOf（彩色任一 → 0 / idle → 1）+ updatedAt 降序混排；截断 MAX_TABS=12 且保 current；
...
- 排序混排逻辑的"彩色状态混合"分支（黄绿蓝并存时的相对顺序）无法在 e2e 中伪造（需真实运行/待审批会话），由代码检视保证；idle-only 退化路径已由 e2e 覆盖排序方向。
