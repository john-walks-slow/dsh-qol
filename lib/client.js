    ...
    function installActiveSessions(ctx) {
      ...
      function ActiveSessionsOverlay(props) {
        ...
        var handleClose = function (id, e) {
          if (e) e.stopPropagation();
          // The last visible tab stays: closing it would only churn a blank
          // session ("+" is the way to start fresh).
          ...
          if (id === current) {
            ...
            if (tabs.length > 0) {
              ...
              // No tabs left — go to new session
              var uiWs = null;
              try { uiWs = ctx.get("uiWorkspace"); } catch (err) {}
              if (uiWs && uiWs.startSession) {
                uiWs.startSession();
              } else {
                try { sService = ctx.get("sessions"); } catch (err) { sService = null; }
                ...
                React.createElement(
                  ...
                  displaySessions.map(function (sess) {
                    ...
                    return React.createElement(
                      ...
                      displaySessions.length > 1 ? React.createElement(
                        "span",
                        {
