...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var FEATURES = [
      ...
      { id: "active-tabbar",   label: "活跃会话 Tab Bar", hint: "页面顶部横向展示活跃会话 Tab，未读/运行中状态置顶，一键直达，防误拉键盘；中键/×关闭", default: true, kind: "mixed" },
    ...
    var CSS = [
      ...
      "html[data-qol-sidebar-overlay=\"on\"]:has([aria-modal=\"true\"]) [data-qol-appframe] > [class*=\"_sidebarCol\"] {",
    ...
    function gestureShouldSkip(target) {
      ...
      if (target.closest("textarea, input, select, [contenteditable]") !== null) return true;
    ...
    // sidebarOpen: dual-attribute recognition. The left sidebar is open when
    // the frame carries neither data-sidebar-collapsed (current name) nor
    // data-rightbar-collapsed (the 0.1.5 rename of the right/details column —
    // recognized too so a future rename that repurposes the frame attribute
    // keeps the direction logic sane). data-rightbar-fullscreen (0.1.5) is an
    ...
    function onToggle(id, on) {
      ...
      // sidebar-gesture / switch-collapse / switch-nofocus / settings-remember-tab
      // read the attribute or isOn() at event/mutation time; no re-arm needed.
    ...
    function installActiveSessions(ctx) {
      ...
      function ActiveSessionsOverlay(props) {
        ...
        React.useEffect(function () {
          if (!current) return;
          var tabs = loadOpenTabs();
          if (tabs.indexOf(current) !== -1) return; // already open, no reorder
        ...
        // Only show sessions that are in open tabs — filter + sort by recency
        var openTabsSet = new Set(openTabs);
        var displaySessions = validSessions.filter(function (s) {
          return openTabsSet.has(s.id);
        });
        ...
        var railSessions = validSessions.slice(0, 10);
        ...
        var handleClose = function (id, e) {
          ...
          // Closing the last visible tab lands on a fresh 新会话 — except
          // when that tab already IS a blank 新会话 (closing it would only
          // churn another blank, so it stays).
          if (remaining.length === 0) {
            var item = byId[id];
            if (item && item.blank) return;
            var lastTabs = loadOpenTabs().filter(function (tid) { return tid !== id; });
            saveOpenTabs(lastTabs);
            setOpenTabs(lastTabs);
            _armSuppress();
            startNewSession();
            return;
          }
          ...
          saveOpenTabs(tabs);
          setOpenTabs(tabs);
          // If closing the current tab, switch to the previous one (or new session)
          if (id === current) {
            _armSuppress(); // the induced switch must not pop the IME either
            if (tabs.length > 0) {
              var sService = null;
              try { sService = ctx.get("sessions"); } catch (err) {}
              if (sService) sService.open(tabs[tabs.length - 1]);
            } else {
              startNewSession();
            }
          }
        ...
        // Blank sessions: the host's displayTitle falls back to the workspace
        // name — "新会话" is what they are (matches the sidebar's row label).
                ...
                React.createElement(
                  ...
                  displaySessions.map(function (sess) {
                    ...
                    return React.createElement(
                      ...
                      (displaySessions.length > 1 || !sess.blank) ? React.createElement(
                        "span",
                        {
