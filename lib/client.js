...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var FEATURES = [
      ...
      { id: "settings-remember-tab", label: "设置页记忆页签", hint: "打开设置时自动恢复上次选中的页签，避免每次重置回 General", default: false, kind: "js" },
      ...
      { id: "active-tabbar",   label: "活跃会话 Tab Bar", hint: "页面顶部横向展示会话 Tab，一键直达，防误拉键盘；新会话不占 tab。显示模式：标准（打开顺序，可关闭）/ 最近活跃（按状态与活跃时间自动排序）", default: true, kind: "mixed" },
      ...
      return React.createElement("div", { className: "dsh-qol-moderow" },
        React.createElement("div", { style: { flex: "1 1 auto", minWidth: 0 } },
          ...
          React.createElement("div", { style: { fontSize: "12px", lineHeight: "16px", color: "var(--dsw-alias-label-tertiary, GrayText)", marginTop: "2px" } },
            mode === "recent"
              ? "最近活跃：全部会话按状态（待处理/完成/运行中混排优先）与活跃时间自动排序，无关闭按钮"
    ...
    function QolPanel() {
      ...
      for (var i = 0; i < FEATURES.length; i++) {
        var f = FEATURES[i];
        rows.push(React.createElement(ToggleRow, {
          key: f.id, id: f.id, label: f.label, hint: f.hint, rerender: rerender
        }));
        if (f.id === "active-tabbar" && isOn("active-tabbar")) {
          rows.push(React.createElement(ModeRow, { key: "tabbar-mode", rerender: rerender }));
        }
      }
    ...
    function installActiveSessions(ctx) {
      ...
      function ActiveSessionsOverlay(props) {
        ...
        if (isRecent) {
          ...
          // open-tabs tracking. Rank: colored states (pending 黄 / completed
          // 绿 / running 蓝) are NOT tiered against each other — they mix by
          // recency as one group, idle (灰) sessions follow; cap at
          // MAX_TABS, keeping the current session visible.
