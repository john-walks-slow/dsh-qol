...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var _caretPointerAt = 0;    // last pointerdown inside the composer
    ...
    function caretObserve(el) {
      ...
      _caretObserver = new MutationObserver(function (muts) {
        ...
        for (var i = 0; i < muts.length; i++) {
          ...
          removed += muts[i].removedNodes.length;
        }
      ...
      _caretObserver.observe(el, { childList: true, subtree: true });
    }
    ...
    function installCaretDebug() {
      ...
      var onPointerDown = function (event) {
        if (inComposer(event.target)) _caretPointerAt = Date.now();
      };
      var onSelectionChange = function () {
        ...
        caretPush("sel", "f=" + s.f + "/" + s.len + " a=" + s.a);
        // Teleport detection: focus offset jumps to a boundary (end or start)
        // by ≥4 chars in one change, with no user tap on the composer in the
        // last 400ms (a tap legitimately moves the caret anywhere). Committed
        // pinyin phrases are excluded: typing at the end means the previous
        // offset was already at the end.
        if (p !== null && Date.now() - _caretPointerAt > 400) {
          if (p.f < p.len - 1 && s.f >= s.len - 1 && s.f - p.f >= 4) {
            caretPush("JUMP-DETECTED", "end f " + p.f + "→" + s.f + " len=" + s.len);
            caretPersist("jump-to-end f" + p.f + "→" + s.f + "/len" + s.len);
          } else if (p.f > 4 && s.f <= 1 && p.f - s.f >= 4) {
