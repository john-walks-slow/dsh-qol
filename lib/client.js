...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var FEATURES = [
      ...
      { id: "workspace-pin",  label: "工作区菜单置顶",     hint: "侧边栏工作区行的 ⋯ 菜单新增「置顶」：一键把该工作区固定到列表最前（等价拖到顶部，顺序云端持久化）；已在最前时显示「已置顶」", default: true, kind: "js", group: "nav" },
      // 输入与键盘
    ...
    // Adds a 置顶 (Pin to top) entry to the workspace ROW menu — the ⋯ menu
    // with 重命名/删除工作区 in the sidebar. Clicking it moves the workspace
    // to the top of the sidebar list via the host's reorder RPC (same effect
    // as dragging it above the first row; the order persists host-side).
    ...
    function installWorkspacePin(ctx) {
      ...
      // Row title → { id, atTop, firstId }; null when unknown (ungrouped
      // bucket, list not ready yet, or title matches nothing).
      function resolveTarget() {
        if (!workspaces || !lastRow || !lastRow.title) return null;
        var snapshot = null;
        try { snapshot = workspaces.list.getSnapshot(); } catch (err) { return null; }
        var items = snapshot && snapshot.items ? snapshot.items : [];
        for (var i = 0; i < items.length; i++) {
          if (items[i] && items[i].title === lastRow.title) {
            return { id: items[i].workspaceId, atTop: i === 0, firstId: i === 0 ? void 0 : items[0].workspaceId };
          }
        }
        return null;
      }
      ...
      function buildPinItem(menu, protoItem, target) {
        var isZh = menuHasZh(menu);
        var label = target.atTop ? (isZh ? "已置顶" : "Pinned") : (isZh ? "置顶" : "Pin to top");
        ...
        btn.innerHTML =
          '<span class="' + iconCls + '"><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M16 9V4h1c.55 0 1-.45 1-1s-.45-1-1-1H7c-.55 0-1 .45-1 1s.45 1 1 1h1v5c0 1.66-1.34 3-3 3v2h5.97v7l1 1 1-1v-7H19v-2c-1.66 0-3-1.34-3-3z"/></svg></span>' +
          '<span class="' + labelCls + '">' + label + '</span>';
        if (target.atTop) {
          btn.setAttribute("aria-disabled", "true");
          btn.style.opacity = "0.55";
          btn.style.cursor = "default";
          btn.title = isZh ? "该工作区已在列表最前" : "Already at the top of the list";
        } else {
          btn.title = isZh ? "固定到工作区列表最前" : "Pin this workspace to the top of the sidebar list";
        }
      ...
      function maybeInject(menu) {
        ...
        var wrap = buildPinItem(menu, items[0], target);
        var btn = wrap.firstChild;
        btn.addEventListener("click", function (e) {
          e.stopPropagation();
          e.preventDefault();
          var t2 = resolveTarget();
          if (!t2 || t2.atTop) return;
          var p = null;
          try { p = workspaces.insertBefore(t2.id, t2.firstId); } catch (err) { return; }
          if (p && typeof p.catch === "function") p.catch(function (err) { console.warn("[dsh-qol] 工作区置顶失败:", err && err.message); });
          closeMenu();
        });
    ...
    function GroupHeader(props) {
      ...
      return React.createElement("div", {
        ...
        React.createElement("span", { className: "dsh-qol-group-count" }, props.count + " 项")
      );
    }
    ...
    function QolPanel() {
      ...
      for (var g = 0; g < GROUPS.length; g++) {
        ...
        sections.push(React.createElement("div", {
          ...
          React.createElement(GroupHeader, { id: grp.id, label: grp.label, count: rowCount, collapsed: !!collapsedGroups[grp.id], rerender: rerender }),
          kids
        ));
      }
