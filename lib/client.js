...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Tab bar display mode: both modes share the same open/close tab set and
    // close interactions; they differ ONLY in ordering — "standard" (fixed
    // open order) or "recent" (status groups + recency, see comparator).
      ...
      return React.createElement("div", { className: "dsh-qol-moderow" },
        React.createElement("div", { style: { flex: "1 1 auto", minWidth: 0 } },
          ...
          React.createElement("div", { style: { fontSize: "12px", lineHeight: "16px", color: "var(--dsw-alias-label-tertiary, GrayText)", marginTop: "2px" } },
            mode === "recent"
              ? "最近活跃：同一组可开可关的页签，按活跃排序——待处理/完成/运行中排前（最久未动最左），空闲排后（最新最左）"
              : "标准：同一组可开可关的页签，按打开顺序固定排列")
        ),
    ...
    function installActiveSessions(ctx) {
      ...
      function ActiveSessionsOverlay(props) {
        ...
        // reorder). This set is the tab source for BOTH display modes —
        // they differ only in ordering.
        React.useEffect(function () {
          if (!current) return;
        ...
        var displaySessions;
        // Both modes show the SAME tab set — the sessions the user has
        // opened (open/close state, close buttons, MAX_TABS window). The
        // only difference between modes is the order below.
        var openTabsSet = new Set(openTabs);
        displaySessions = validSessions.filter(function (s) {
          return openTabsSet.has(s.id) && isReal(s);
        });
        if (isRecent) {
          // 最近活跃 order: colored states (pending 黄 / completed 绿 /
          // running 蓝, not tiered against each other) first, idle (灰)
          // after. Within each group by updatedAt, OPPOSITE directions:
          // colored oldest-first (the longest-waiting tab is leftmost — a
          // FIFO work queue), idle newest-first (the most recently used
          // tab is leftmost).
          ...
          // 标准 order: fixed by open time — switching sessions never
          // reorders tabs.
          displaySessions.sort(function (a, b) {
        ...
        var handleClose = function (id, e) {
          ...
          // Closing the current tab: land on a remaining VISIBLE tab
          ...
          if (remaining.length > 0) {
            var sService = null;
            try { sService = ctx.get("sessions"); } catch (err) {}
            // Landing tab after closing the current one: 标准 → most
            // recently opened (rightmost); 最近活跃 → most recently active,
            // matching the mode's own ordering idea.
            var landing = remaining[remaining.length - 1];
            if (isRecent) {
              for (var li = 0; li < remaining.length; li++) {
                if ((remaining[li].updatedAt || 0) > (landing.updatedAt || 0)) landing = remaining[li];
              }
            }
            if (sService) sService.open(landing.id);
          } else {
            startNewSession();
          }
        };
              ...
              displaySessions.map(function (sess) {
                ...
                return React.createElement(
                  "div",
                  {
                    ...
                    // itself rides on auxclick (both modes).
