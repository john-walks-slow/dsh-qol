...
//   - 13 html[data-qol-*] attributes set (defaults all true)
...
//     skip when modalOpen / on form controls / below threshold
...
// --- inject the plugin via mock harness ---
const result = await page.evaluate((pluginSrc) => {
  const log = [];
  let captured = null;
...
check(result.attrs.length === 13 && result.attrs.includes('data-qol-settings-remember-tab'), '13 feature attributes set including settings-remember-tab (got ' + result.attrs.length + ')');
...
if (isMobile && !result.error) {
  ...
  // first ensure sidebar is collapsed (it is by default on mobile)
  ...
  console.log('  sidebar collapsed initially:', wasCollapsed);
