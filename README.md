...
dsh（DeepSeek Harness）Web GUI 体验优化插件：会话 Tab Bar、侧栏滑动开合、输入法/键盘适配、触摸反馈、设置页全屏重写等 **多项 QoL 功能**，每项都可在 **设置 → QoL** 独立开关，即时生效、按浏览器持久保存。移动端为主，部分功能（Tab Bar、状态动画等）桌面端同样生效。

![dsh-qol: mobile tab bar and desktop views of the DeepSeek Harness web UI](assets/hero.png)

## 功能

设置 → **QoL** 中的全部功能开关按 4 组组织（点击组标题可折叠）：

### 导航与切换

| 功能 | 说明 | 默认 |
|---|---|---|
| 活跃会话 Tab Bar | 页面顶部横向展示会话 Tab，一键直达，防误拉键盘；**新会话不占 tab**（点 ＋/切新会话仅取消选中，发出首条消息后才出现）；**+ 固定在标签栏最右**（不随标签滚走）。**标签显示模式**（设置 → QoL 内切换，见下）：两种模式共用同一组"打开过的会话"页签（均可中键/×关闭、状态实时更新），仅排序不同——**标准** = 按打开顺序固定排列；**最近活跃** = 按活跃排序，带状态点的（黄 = 待处理 / 绿 = 完成未读 / 蓝 = 运行中）整体排前、区内**最久未动的在最左**（FIFO 处理队列），灰色空闲会话排后、区内**最新在最左** | 开（模式默认标准） |
...
| 上一条用户消息按钮 | 右下角"回到底部"按钮上方新增一个圆形按钮，点击滚动到上一条（视口上方最近的一条）用户消息，快速回看历史；再点一次继续上跳，长对话逐条回溯 | 开 |
| 工作区菜单置顶 | 侧边栏工作区行的 ⋯ 菜单新增「置顶」：**支持多个工作区置顶**，置顶的工作区固定排在列表最前（顺序云端持久化，刷新不丢），行右侧显示小图钉图标；已置顶的菜单项变「取消置顶」，点击移出置顶区 | 开 |
...
2. 设置 → **QoL**：全部功能开关按 4 组组织（导航与切换 / 输入与键盘 / 触摸与反馈 / 显示与设置页），点击组标题可折叠；每个开关点击**即时生效**，无需刷新页面。
3. 配置自动持久保存到浏览器 `localStorage`（键 `dsh.qol.v1`），仅本浏览器生效；清掉该键即恢复默认配置。分组折叠状态单独存于键 `dsh.qol.groups`。
...
- 新增功能：在 `lib/client.js` 的 `FEATURES` 注册表加一条（注明所属 `group`，组序见 `GROUPS`）+ 对应 CSS 段/JS 钩子。
